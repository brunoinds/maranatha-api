...
class InventoryWarehouseController extends Controller
{
    ...
    public function listOutcomeResumeAnalisys2(InventoryWarehouse $warehouse)
    {
        $validated = request()->validate([
            'products' => 'required|array',
            'products.*.product_id' => 'required|exists:inventory_products,id',
    ...
    public function listOutcomeResumeAnalisys(InventoryWarehouse $warehouse)
    {
        ...
        $productsResume = [];
        $batchSize = 900; // Limit for SQLite parameter restriction
    ...
    protected function batchQuery($queryFunc, array $ids, $batchSize = 900)
    {
        $results = [];
        $isNumeric = false;
